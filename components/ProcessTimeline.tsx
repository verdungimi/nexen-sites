...
export default function ProcessTimeline() {
  ...
  // Calculate total height of timeline (approximate: 5 items * spacing)
  const timelineHeight = steps.length * 200; // Approximate height per item with spacing
  const progressHeight = timelineHeight * scrollProgress;

  return (
    <div ref={sectionRef} className="relative">
      {/* Scroll progress line - single vertical line for entire timeline */}
      <div className="absolute left-6 top-10 bottom-10 w-0.5 bg-[#7C5CFF]/20 hidden md:block" style={{ height: `${timelineHeight}px` }}>
        {/* Progress fill */}
        <div
          className="absolute top-0 left-0 w-full bg-gradient-to-b from-[#7C5CFF] via-[#50AEDF] to-[#7C5CFF] transition-all duration-300 ease-out"
          style={{
